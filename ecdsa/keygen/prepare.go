...
func GeneratePreParams(timeout time.Duration, optionalConcurrency ...int) (*LocalPreParams, error) {
	...
	sgpCh := make(chan []*utils.GermainSafePrime, 1)

	// 4. generate Paillier public key E_i, private key and proof
	go func(ch chan<- *paillier.PrivateKey) {
		utils.Logger.Info("generating the Paillier modulus, please wait...")
		start := time.Now()
		...
		utils.Logger.Infof("paillier modulus generated. took %s\n", time.Since(start))
		ch <- PiPaillierSk
	}(paiCh)

	// 5-7. generate safe primes for ZKPs used later on
	go func(ch chan<- []*utils.GermainSafePrime) {
		var err error
		utils.Logger.Info("generating the safe primes for the signing proofs, please wait...")
		start := time.Now()
		sgps, err := utils.GetRandomSafePrimesConcurrent(safePrimeBitLen, 2, timeout, concurrency)
		if err != nil {
			ch <- nil
			return
		}
		utils.Logger.Infof("safe primes generated. took %s\n", time.Since(start))
		ch <- sgps
	}(sgpCh)
	...
	var sgps []*utils.GermainSafePrime
	var paiSK *paillier.PrivateKey
consumer:
	for {
		select {
		case <-logProgressTicker.C:
			utils.Logger.Info("still generating primes...")
		case sgps = <-sgpCh:
			if sgps == nil ||
	...
	modNTildeI := utils.ModInt(NTildei)

	p, q := sgps[0].Prime(), sgps[1].Prime()
	modPQ := utils.ModInt(new(big.Int).Mul(p, q))
	f1 := utils.GetRandomPositiveRelativelyPrimeInt(NTildei)
	alpha := utils.GetRandomPositiveRelativelyPrimeInt(NTildei)
