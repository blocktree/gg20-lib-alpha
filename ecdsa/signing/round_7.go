...
func (round *round7) Start() *gg20.Error {
	if round.started {
	...
	if round.abortingT5 {
		utils.Logger.Infof("round 7: Abort Type 5 code path triggered")
	outer:
		for j, msg := range round.temp.signRound6Messages {
			...
			if !ok {
				utils.Logger.Warnf("round 7: unexpected success message while in aborting mode: %+v", r6msgInner)
			...
			gammaJG := crypto.ScalarBaseMult(gg20.EC(), gammaJ)
	...
	bigR := crypto.NewECPointNoCurveCheck(gg20.EC(), bigRX, bigRY)

	h, err := crypto.ECBasePoint2(gg20.EC())
	if err != nil {
		return round.WrapError(err, Pi)
	}

	bigSJ := make(map[string]*utils.ECPoint, len(round.temp.signRound6Messages))
	...
	if y := round.key.ECDSAPub; !bigSJProducts.Equals(y) {
		...
		utils.Logger.Warnf("round 7: consistency check failed: y != bigSJ products, entering Type 7 identified abort")
...
func (round *round7) Update() (bool, *gg20.Error) {
...
func (round *round7) CanAccept(msg gg20.ParsedMessage) bool {
	// Collect messages for the full online protocol OR identified abort of type 7.
...
func (round *round7) NextRound() gg20.Round {
