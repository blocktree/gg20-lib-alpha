...
func FinalizeGetAndVerifyFinalSig(
	...
	ourP *gg20.PartyID,
	ourSI *big.Int,
	otherSIs map[*gg20.PartyID]*big.Int,
) (*SignatureData, *btcec.Signature, *gg20.Error) {
	if len(otherSIs) == 0 {
	...
	bigR, err := crypto.NewECPoint(gg20.EC(),
	...
	r, s := bigR.X(), ourSI
	culprits := make([]*gg20.PartyID, 0, len(otherSIs))
	for Pj, sJ := range otherSIs {
		...
		bigRBarJ, err := crypto.NewECPoint(gg20.EC(),
		...
		bigSI, err := crypto.NewECPoint(gg20.EC(),
	...
	signature := new(utils.ECSignature)
...
func FinalizeWrapError(err error, victim *gg20.PartyID, culprits ...*gg20.PartyID) *gg20.Error {
	return gg20.NewError(err, TaskNameFinalize, 8, victim, culprits...)
}
...
func (round *finalization) Start() *gg20.Error {
	if round.started {
	...
	culprits := make([]*gg20.PartyID, 0, len(round.temp.signRound6Messages))

	// Identifiable Abort Type 7 triggered during Phase 6 (GG20)
	if round.abortingT7 {
		utils.Logger.Infof("round 8: Abort Type 7 code path triggered")
		q := gg20.EC().Params().N
	...
	outer:
		for j, msg := range round.temp.signRound7Messages {
			...
			if !ok {
				utils.Logger.Warnf("round 8: unexpected success message while in aborting mode: %+v", r7msgInner)
				culprits = append(culprits, Pj)
				continue
			}
			...
			mus := utils.ByteSlicesToBigInts(r7msg.GetMuIJ())
			muRands := utils.ByteSlicesToBigInts(r7msg.GetMuRandIJ())
			...
			for k, mu := range mus {
				...
				gMus[j][k] = crypto.ScalarBaseMult(gg20.EC(), mu.Mod(mu, q))
			}
		}
		bigR := round.temp.rI
		if 0 < len(culprits) {
		...
		for i, P := range Ps {
			...
			if !gSigmaIPfs[i].VerifySigmaI(gg20.EC(), gSigmaI, bigR, bigSI) {
	...
	ourSI := round.temp.sI
	otherSIs := make(map[*gg20.PartyID]*big.Int, len(Ps)-1)
	var multiErr error
	for j, msg := range round.temp.signRound7Messages {
		if j == i {
			continue
...
func (round *finalization) CanAccept(msg gg20.ParsedMessage) bool {
	// not expecting any incoming messages in this round
	return false
}

func (round *finalization) Update() (bool, *gg20.Error) {
	// not expecting any incoming messages in this round
	return false, nil
}

func (round *finalization) NextRound() gg20.Round {
	return nil // finished!
}
