...
func (round *round6) Start() *gg20.Error {
	if round.started {
	...
	errs := make(map[*gg20.PartyID]error)
	bigRBarJProducts := (*crypto.ECPoint)(nil)
	BigRBarJ := make(map[string]*utils.ECPoint, len(round.temp.signRound5Messages))
	...
	if 0 < len(errs) {
		var multiErr error
		culprits := make([]*gg20.PartyID, 0, len(errs))
	...
	}
	{
		...
		if bigRBarJProducts.X().Cmp(gX) != 0 || bigRBarJProducts.Y().Cmp(gY) != 0 {
			...
			utils.Logger.Warnf("round 6: consistency check failed: g != R products, entering Type 5 identified abort")
	...
	{
		sigmaPf, err := zkp.NewECSigmaIProof(gg20.EC(), sigmaI, bigR, bigSI)
		if err != nil {
...
func (round *round6) Update() (bool, *gg20.Error) {
...
func (round *round6) CanAccept(msg gg20.ParsedMessage) bool {
...
func (round *round6) NextRound() gg20.Round {
	round.started = false
