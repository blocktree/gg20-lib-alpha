...
func (round *round3) Start() *gg20.Error {
	...
	errChs := make(chan *gg20.Error, (len(round.Parties().IDs())-1)*2)
	wg := sync.WaitGroup{}
	...
	wg.Wait()
	close(errChs)
	culprits := make([]*gg20.PartyID, 0, len(round.Parties().IDs()))
	...
	round.temp.r7AbortData.MuIJ = utils.BigIntsToBytes(muIJRecs)
	round.temp.r7AbortData.MuRandIJ = utils.BigIntsToBytes(muRandIJ)
	...
	lI := utils.GetRandomPositiveInt(q)
	h, err := crypto.ECBasePoint2(gg20.EC())
	if err != nil {
		return round.WrapError(err, Pi)
	}
	hLI := h.ScalarMult(lI)
	gSigmaI := crypto.ScalarBaseMult(gg20.EC(), sigmaI)
