...
func ProveBobWC(pk *paillier.PublicKey, NTilde, h1, h2, c1, c2, x, y, r *big.Int, X *crypto.ECPoint) (*ProofBobWC, error) {
	...
	// 1.
	alpha := utils.GetRandomPositiveInt(q3)

	// 2.
	rho := utils.GetRandomPositiveInt(qNTilde)
	sigma := utils.GetRandomPositiveInt(qNTilde)
	tau := utils.GetRandomPositiveInt(qNTilde)

	// 3.
	rhoPrm := utils.GetRandomPositiveInt(q3NTilde)

	// 4.
	beta := utils.GetRandomPositiveRelativelyPrimeInt(pk.N)
	gamma := utils.GetRandomPositiveRelativelyPrimeInt(pk.N)

	// 5.
	u := crypto.NewECPointNoCurveCheck(gg20.EC(), zero, zero) // initialization suppresses an IDE warning
	if X != nil {
		u = crypto.ScalarBaseMult(gg20.EC(), alpha)
	}

	// 6.
	modNTilde := utils.ModInt(NTilde)
	...
	{ // must use RejectionSample
		...
		if X == nil {
			eHash = utils.SHA512_256i(append(pk.AsInts(), c1, c2, z, zPrm, t, v, w)...)
		} else {
			eHash = utils.SHA512_256i(append(pk.AsInts(), X.X(), X.Y(), c1, c2, u.X(), u.Y(), z, zPrm, t, v, w)...)
		}
		e = utils.RejectionSample(q, eHash)
	}

	// 13.
	modN := utils.ModInt(pk.N)
	s := modN.Exp(r, e)
	s = modN.Mul(s, beta)
...
func ProofBobWCFromBytes(bzs [][]byte) (*ProofBobWC, error) {
	...
	point, err := crypto.NewECPoint(gg20.EC(),
...
func ProofBobFromBytes(bzs [][]byte) (*ProofBob, error) {
	if !utils.NonEmptyMultiBytes(bzs, ProofBobBytesParts) &&
		!utils.NonEmptyMultiBytes(bzs, ProofBobWCBytesParts) {
...
func (pf *ProofBobWC) Verify(pk *paillier.PublicKey, NTilde, h1, h2, c1, c2 *big.Int, X *crypto.ECPoint) bool {
	...
	{ // must use RejectionSample
		...
		if X == nil {
			eHash = utils.SHA512_256i(append(pk.AsInts(), c1, c2, pf.Z, pf.ZPrm, pf.T, pf.V, pf.W)...)
		} else {
			eHash = utils.SHA512_256i(append(pk.AsInts(), X.X(), X.Y(), c1, c2, pf.U.X(), pf.U.Y(), pf.Z, pf.ZPrm, pf.T, pf.V, pf.W)...)
		}
		e = utils.RejectionSample(q, eHash)
	}
	...
	if X != nil {
		s1ModQ := new(big.Int).Mod(pf.S1, gg20.EC().Params().N)
		gS1 := crypto.ScalarBaseMult(gg20.EC(), s1ModQ)
	...
	{ // 7.
		modNSq := utils.ModInt(pk.NSquare())
