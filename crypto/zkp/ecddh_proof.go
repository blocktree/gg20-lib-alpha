...
func NewECDDHProof(wit ECDDHWitness, st ECDDHStatement) ECDDHProof {
	...
	s := utils.GetRandomPositiveInt(st.Curve.Params().N)
	a1 := crypto.ScalarBaseMult(st.Curve, s)
	a2 := st.G2.ScalarMult(s)
	e := utils.SHA512_256(g1.Bytes(), st.H1.Bytes(), st.G2.Bytes(), st.H2.Bytes(), a1.Bytes(), a2.Bytes())
...
func (pf *ECDDHProof) Verify(st ECDDHStatement) bool {
	...
	e := utils.SHA512_256(g1.Bytes(), st.H1.Bytes(), st.G2.Bytes(), st.H2.Bytes(), pf.A1.Bytes(), pf.A2.Bytes())
