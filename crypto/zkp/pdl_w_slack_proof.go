...
func NewPDLwSlackProof(wit PDLwSlackWitness, st PDLwSlackStatement) PDLwSlackProof {
	...
	beta := new(big.Int).Add(one, utils.GetRandomPositiveInt(nSubOne))
	...
	e := utils.SHA512_256i(st.G.X(), st.G.Y(), st.Q.X(), st.Q.Y(), st.CipherText, z, u1.X(), u1.Y(), u2, u3)
...
func (pf PDLwSlackProof) Verify(st PDLwSlackStatement) bool {
	...
	e := utils.SHA512_256i(st.G.X(), st.G.Y(), st.Q.X(), st.Q.Y(), st.CipherText, pf.Z, pf.U1.X(), pf.U1.Y(), pf.U2, pf.U3)
...
func UnmarshalPDLwSlackProof(bzs [][]byte) (*PDLwSlackProof, error) {
	...
	p.Z = parsed[0][0]
	U1, err := crypto.NewECPoint(gg20.EC(), parsed[1][0], parsed[1][1])
	if err != nil {
		return nil, err
	}
	p.U1 = U1
