...
func Create(threshold int, secret *big.Int, indexes []*big.Int) (Vs, Shares, error) {
	...
	ids, err := CheckIndexes(gg20.EC(), indexes)
	if err != nil {
		return nil, nil, err
	}
	num := len(indexes)
	...
	v := make(Vs, len(poly))
	for i, ai := range poly {
		v[i] = crypto.ScalarBaseMult(gg20.EC(), ai)
	}
...
func (share *Share) Verify(threshold int, vs Vs) bool {
	...
	for j := 1; j <= threshold; j++ {
		...
		// v = v * v_j^t
		vjt := vs[j].SetCurve(gg20.EC()).ScalarMult(t)
		v, err = v.SetCurve(gg20.EC()).Add(vjt)
		if err != nil {
			return false
		}
	}
	sigmaGi := crypto.ScalarBaseMult(gg20.EC(), share.Share)
...
func samplePolynomial(threshold int, secret *big.Int) []*big.Int {
	...
	for i := 1; i <= threshold; i++ {
		ai := utils.GetRandomPositiveInt(q)
		v[i] = ai
	}
	return v
}
