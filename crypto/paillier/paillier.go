...
import (
	...
	crypto2 "github.com/blocktree/gg20-lib-alpha/crypto"
...
func GenerateKeyPair(modulusBitLen int, timeout time.Duration, optionalConcurrency ...int) (privateKey *PrivateKey, publicKey *PublicKey, err error) {
	...
	var P, Q, N *big.Int
	{
		tmp := new(big.Int)
		for {
			sgps, err := utils.GetRandomSafePrimesConcurrent(modulusBitLen/2, 2, timeout, concurrency)
			if err != nil {
				return nil, nil, err
			}
...
func (pk *PublicKey) EncryptAndReturnRandomness(m *big.Int) (c *big.Int, x *big.Int, err error) {
	...
	modNSq := utils.ModInt(pk.NSquare())
	x = utils.GetRandomPositiveRelativelyPrimeInt(pk.N)
	// 1. gamma^m mod N2
	Gm := modNSq.Exp(pk.Gamma(), m)
	// 2. x^N mod N2
	xN := modNSq.Exp(x, pk.N)
...
func (pk *PublicKey) HomoMult(m, c1 *big.Int) (*big.Int, error) {
	...
	// cipher^m mod N2
	return utils.ModInt(NSq).Exp(c1, m), nil
}
...
func (pk *PublicKey) HomoAdd(c1, c2 *big.Int) (*big.Int, error) {
	...
	// c1 * c2 mod N2
	return utils.ModInt(NSq).Mul(c1, c2), nil
}
...
func (sk *PrivateKey) DecryptAndRecoverRandomness(c *big.Int) (m, x *big.Int, err error) {
	...
	modN := utils.ModInt(sk.N)
	modNSq := utils.ModInt(sk.NSquare())
	modPhiN := utils.ModInt(sk.PhiN)
	...
	for i < m {
		...
		for j := 0; j < blocks; j++ {
			go func(j int) {
				...
				hash := utils.SHA512_256(ib, jBz, nb, kb, sXb, sYb, Nb)
				chs[j] <- hash
			}(j)
		}
		...
		if utils.IsNumberInMultiplicativeGroup(N, ret[i]) {
			i++
		} else {
			n++
		}
