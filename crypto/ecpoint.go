...
func NewECPointFromProtobuf(p *utils.ECPoint) (*ECPoint, error) {
	if p == nil || p.GetX() == nil || p.GetY() == nil {
		return nil, errors.New("nil protobuf point provided")
	}
	return NewECPoint(gg20.EC(), new(big.Int).SetBytes(p.GetX()), new(big.Int).SetBytes(p.GetY()))
}
...
func (p *ECPoint) ToProtobufPoint() *utils.ECPoint {
	return &utils.ECPoint{
		X: p.X().Bytes(),
		Y: p.Y().Bytes(),
	}
}
